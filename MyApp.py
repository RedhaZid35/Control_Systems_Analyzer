...
class MyFrame (wx.Frame):
    def __init__(self, parent):
        ...
        self.Auto_timing = wx.CheckBox(self, wx.ID_ANY, "Auto Timing", wx.DefaultPosition, wx.DefaultSize, 0)

        self.Time_lbl = wx.StaticText(self, wx.ID_ANY, "Simulation time", wx.DefaultPosition, wx.DefaultSize, 0)
        self.Time_txt = wx.TextCtrl(self, wx.ID_ANY,"1", wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.U_check = wx.CheckBox(self, wx.ID_ANY, "Plot Input", wx.DefaultPosition, wx.DefaultSize, 0)
...
# todo : plllllloooooot
        ...
        self.Implement_PID_button = wx.Button(
            self, wx.ID_ANY, u"Implement PID controller", wx.DefaultPosition, wx.DefaultSize, 0)
        ...
        self.U_val_txt.SetMinSize(wx.Size(100, -1))
        self.Time_txt.SetMinSize(wx.Size(73, -1))
        ...
        _G_sizer.Add(self.G_num_txt, 0, wx.ALL | wx.EXPAND, 5)
        _G_sizer.Add(self.G_den_txt, 0, wx.ALL | wx.EXPAND, 5)

        _H_sizer.Add(self.H_lable, 0, wx.ALL, 5)
        _H_sizer.Add(self.H_num_txt, 0, wx.ALL | wx.EXPAND, 5)
        _H_sizer.Add(self.H_den_txt, 0, wx.ALL | wx.EXPAND, 5)
        ...
        _Body_left.Add(U_val_sizer_parent, 0, wx.EXPAND, 5)
        _Body_left.Add(__PID_Sizer, 0, wx.EXPAND, 5)
        _Body_left.Add(__Status_sizer, 1, wx.EXPAND, 5)
# todo : plooot
        self.__Plot_sizer.Add(self.canvas, 1, wx.EXPAND | wx.ALL, 5)
        self.__Plot_sizer.Add(self.toolbar, 0,wx.EXPAND | wx.ALL, 5)

        _Body_right.Add(self.__Plot_sizer, 1, wx.EXPAND| wx.LEFT | wx.RIGHT, 5)

        Header.Add(_G_sizer, 1, wx.ALL | wx.EXPAND, 5)
        Header.Add(_H_sizer, 1, wx.ALL | wx.EXPAND, 5)
        Header.Add(_Loop_sizer, 1, wx.ALL | wx.EXPAND, 5)

        Body.Add(_Body_left, 0, wx.EXPAND | wx.LEFT, 10)
        Body.Add(_Body_right, 1, wx.EXPAND)

        Footer .Add(self.Simulate_button, 1, wx.ALL, 5)
        Footer .Add(self.Implement_PID_button, 1, wx.ALL, 5)
        Footer .Add(self.Plot_bode_button, 1, wx.ALL, 5)
        Footer .Add(self.Clear_canvas_button, 1, wx.ALL, 5)

        Main_sizer.Add(Header, 0, wx.ALL | wx.EXPAND, 10)
        Main_sizer.Add(Body, 1, wx.RIGHT | wx.LEFT | wx.EXPAND, 10)
        Main_sizer.Add(Footer, 0, wx.ALL | wx.EXPAND, 10)
    ...
    def plot_u(self,event):
        if self.U_check.IsChecked():
            U_val = abs([float(i) for i in str(self.U_val_txt.GetValue()).strip().split()][0])
            pass
        else:
            pass
...
# todo ploooooootototot
    ...
    def implement_pid(self, event):
        try:
            ...
            new_system = self.my_system.pid_version(
                ...
                Kd = Kd[0] if self.D_check.IsChecked() and Kd else 0,
                )
    ...
    def update_lbls(self,sys, test):
        self.my_lables["Stability"].SetLabel(f"Stability : {sys.is_stable_by_poles_method()}")
    ...
    def calculate_step_r(self,sys):
        U_val = abs([float(i) for i in str(self.U_val_txt.GetValue()).strip().split()][0])
        time = [float(i) for i in str(self.Time_txt.GetValue()).strip().split()]
